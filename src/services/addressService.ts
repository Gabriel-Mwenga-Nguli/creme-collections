...
export async function getUserAddresses(userId: string): Promise<Address[]> {
  console.log(`[Demo Mode] Called getUserAddresses for user ${userId}. Returning empty array.`);
  return [];
}

export async function addUserAddress(userId: string, addressData: Omit<Address, 'id' | 'userId'>): Promise<string | null> {
  console.log(`[Demo Mode] Called addUserAddress for user ${userId}. Firestore is disabled.`);
...
export async function updateUserAddress(userId: string, addressId: string, addressData: Partial<Omit<Address, 'id' | 'userId'>>): Promise<boolean> {
  console.log(`[Demo Mode] Called updateUserAddress for user ${userId}. Firestore is disabled.`);
  return true;
}

export async function deleteUserAddress(userId: string, addressId: string): Promise<boolean> {
  console.log(`[Demo Mode] Called deleteUserAddress for user ${userId}. Firestore is disabled.`);
  return true;
}
